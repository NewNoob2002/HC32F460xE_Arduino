...
StarMapView::Create(lv_obj_t* root) {
    ...
    const lv_font_t* font_title = ResourcePool::GetFont("rajdhaniBold_20");
    ...
    // 2x2 Grid for Constellation Counts
    const char* names[4] = {"GPS", "BDS", "GLN", "GAL"};
    lv_color_t colors[4] = {kColorGps, kColorBds, kColorGln, kColorGal};

    for (int i = 0; i < 4; ++i) {
        int row = i / 2;
        int col = i % 2;
        int x_base = col * 90;
        int y_base = 28 + row * 24;

        // Container/cont for item
        lv_obj_t* cont = lv_obj_create(info_cont);
        lv_obj_remove_style_all(cont);
        lv_obj_set_size(cont, 85, 20);
        ...
        lv_obj_align_to(label_val, label_name, LV_ALIGN_OUT_RIGHT_MID, 6, 0);
...
void
StarMapView::UpdateValues(int gps, int bds, int gln, int gal) {
    char buf[16];

    // Update value labels (single number used counts)
    ...
    // Calculate how many dots to show for each based on the counts
    int counts[4] = {gps, bds, gln, gal};
    for (int c = 0; c < 4; ++c) {
        int used = counts[c];
        int show_dots = 0;
        if (used > 0) {
