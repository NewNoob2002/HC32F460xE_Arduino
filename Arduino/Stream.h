...
class Stream : public Print {
  ...
  public:
    ...
    bool
    find(uint8_t* target, size_t length) {
    ...
    bool
    findUntil(uint8_t* target, char* terminator) {
    ...
    bool
    findUntil(uint8_t* target, size_t targetLen, char* terminate, size_t termLen) {
    ...
    size_t
    readBytes(uint8_t* buffer, size_t length) {
  ...
  protected:
    long
    parseInt(char ignore) {
        return parseInt(SKIP_ALL, ignore);
    }

    float
    parseFloat(char ignore) {
