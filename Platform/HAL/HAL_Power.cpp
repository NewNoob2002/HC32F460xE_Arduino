...
bool
ChagrerMp2762_begin(pBatteryInfo_t p_batteryState) {
    // Reset all configure
    mp2762registerReset();
    // Set Charger Current
...
void
ChargerMp2762_update(pBatteryInfo_t p_batteryState) {
...
void
Bat_Comp_Init(Battery_Compensate_t* bat_comp) {
    if (bat_comp == nullptr) {
        return;
    }
...
void
Bat_Comp_CalcOffset(BatteryInfo_t* batteryState) {
    if (batteryState == nullptr || batteryState->Compensate.is_comp_updated == 1) {
        return;
    }
...
uint8_t
Flash_Write_CompOffset(uint8_t offset) {
