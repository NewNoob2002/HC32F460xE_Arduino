...
{
	if (systemInfo.recordInfo.record_status)
		lv_label_set_text_fmt(label_record_rsc_t, "Space: %0.1fMB", systemInfo.recordInfo.record_leftspace);
	else
		lv_label_set_text(label_record_rsc_t, "Space: -MB");
}
...
{
	if (systemInfo.recordInfo.record_status)
		lv_label_set_text_fmt(label_record_name_t, "Name: %s", systemInfo.recordInfo.record_name);
	else
		lv_label_set_text(label_record_name_t, "Name: -");
}
...
void lv_update_record_type_text()
{
	...
	if (systemInfo.recordInfo.record_status)
	{
		if (systemInfo.recordInfo.record_type == 1)
			lv_label_set_text_fmt(label_record_type_t, "Type: XYZ");
		else if (systemInfo.recordInfo.record_type == 2)
			lv_label_set_text_fmt(label_record_type_t, "Type: Rinex 3.02");
	}
	else
	{
		lv_label_set_text(label_record_type_t, "Type: -");
	}
}
...
void lv_update_record_time_text()
{
	...
	if (systemInfo.recordInfo.record_status)
	{
		if (systemInfo.recordInfo.record_interval == 0x00)
			lv_label_set_text_fmt(label_record_time_t, "Time: 15min");
		else
			lv_label_set_text_fmt(label_record_time_t, "Time: %d hour", systemInfo.recordInfo.record_interval);
	}
	else
	{
		lv_label_set_text(label_record_time_t, "Time: -");
	}
	}
