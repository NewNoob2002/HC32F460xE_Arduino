...
void lv_screen8_init(void)
{
    ...
    if (RootDefaultStyle) {
        lv_obj_add_style(scr8, RootDefaultStyle, LV_PART_MAIN);
    }
    lv_obj_t *root        = scr8;
    ...
    lv_label_set_text(label, GET_TEXT(TEXT_HARAWARE_CHECK));
    lv_obj_center(label);
    ui.logo_label = label;
...
#define ANIM_DEF(start_time, obj, attr, start, end)                                       \
    ...
    lv_img_set_src(img_logo, &img_src_RGKLogo);
#else
...
static void onTimer(lv_timer_t *timer)
{
    if (!do_it_once) {
        do_it_once = true;
        lv_obj_fade_out(ui.img_logo, 500, 0);
