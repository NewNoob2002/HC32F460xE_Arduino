...
int32_t
FlashWriteData(uint32_t u32Addr, uint8_t* pu8Buff, uint32_t u32Len) {
...
int32_t
FlashEraseSector(uint32_t u32Addr) {
    int32_t result = LL_OK;
    /* Wait flash ready. */
    while (SET != EFM_GetStatus(EFM_FLAG_RDY)) {
        LOG_INFO("Wait Flash Ready");
...
void
FlashRead(uint32_t Address, unsigned char* pdata, uint32_t len) {
...
uint16_t
FLASH_PageNumber(uint32_t u32Addr) {
