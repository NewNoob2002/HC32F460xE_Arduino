...
int32_t PORT_GetConfig(uint8_t port, uint16_t pin, stc_gpio_init_t *portConf) {
    ...
    for (uint8_t pinPos = 0u; pinPos < 16u; pinPos++) {
        if ((pin & 1U) != 0U) {
            const __IO uint16_t *PCRx = &CM_GPIO->PCRA0 + 0x40ul * port + 0x4ul * pinPos;
            const uint16_t reg_value = READ_REG16(*PCRx);
            // input latch setting
...
int32_t PORT_GetFunc(uint8_t port, uint16_t pin, uint16_t *funcSel, en_functional_state_t *subFuncEn) {
    ...
    for (uint8_t pinPos = 0u; pinPos < 16u; pinPos++) {
        if ((pin & 1U) != 0U) {
            const __IO uint16_t *PFSRx = &CM_GPIO->PFSRA0 + 0x40ul * port + 0x4ul * pinPos;

            // main function
            const uint16_t reg_value = READ_REG16(*PFSRx);
