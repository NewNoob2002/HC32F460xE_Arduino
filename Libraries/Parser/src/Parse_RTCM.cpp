...
uint32_t sempRtcmComputeCrc24q(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    parse->crc = ((parse)->crc << 8) ^
        semp_crc24qTable[data ^ (((parse)->crc >> 16) & 0xff)];
...
bool sempRtcmReadCrc(SEMP_PARSE_STATE* parse, uint8_t data)
{
    ...
    if ((parse->crc == 0) || (parse->badCrc && (!parse->badCrc(parse))))
        ...
        sempPrintf(parse->printDebug,
                   ...
                   parse->parserName, scratchPad->rtcm.message, parse->length,
                   parse->length, parse->buffer[parse->length - 3],
                   parse->buffer[parse->length - 2],
                   parse->buffer[parse->length - 1],
                   (scratchPad->rtcm.crc >> 16) & 0xff,
                   (scratchPad->rtcm.crc >> 8) & 0xff, scratchPad->rtcm.crc & 0xff);
...
bool sempRtcmReadMessage2(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
...
bool sempRtcmReadMessage1(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
...
bool sempRtcmReadLength2(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
...
bool sempRtcmReadLength1(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
...
bool sempRtcmPreamble(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    if (data == 0xd3)
    {
