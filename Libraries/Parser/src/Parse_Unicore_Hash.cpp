...
// Read the linefeed
bool sempUnicoreHashLineFeed(SEMP_PARSE_STATE* parse, const uint8_t data)
{
...
bool sempUnicoreHashCarriageReturn(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    // Don't add the current character to the length
    parse->length -= 1;
...
bool sempUnicoreHashLineTermination(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    // Don't add the current character to the length
    parse->length -= 1;
...
bool sempUnicoreHashChecksumByte(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
...
bool sempUnicoreHashFindAsterisk(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
    if (data == '*')
    {
...
bool sempUnicoreHashFindFirstComma(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
    parse->crc ^= data;
...
bool sempUnicoreHashPreamble(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    auto* scratchPad = static_cast<SEMP_SCRATCH_PAD*>(parse->scratchPad);
    if (data != '#')
        return false;
