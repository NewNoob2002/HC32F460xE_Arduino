...
bool sempUbloxCkB(SEMP_PARSE_STATE *parse, uint8_t data) {
  ...
  if ((badChecksum == false) || (parse->badCrc && (!parse->badCrc(parse))))
    ...
    sempPrintf(
        ...
        parse->parserName, parse->buffer[parse->length - 2],
        parse->buffer[parse->length - 1], scratchPad->ublox.ck_a,
        scratchPad->ublox.ck_b);
...
bool sempUbloxSync2(SEMP_PARSE_STATE *parse, uint8_t data) {
  ...
  if (data != 0x62) {
    ...
    sempPrintf(parse->printDebug, "SEMP %s: UBLOX invalid second sync byte",
               parse->parserName);
