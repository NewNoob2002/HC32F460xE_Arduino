...
uint32_t sempUnicoreBinaryComputeCrc(SEMP_PARSE_STATE* parse, const uint8_t data)
{
...
void sempUnicoreBinaryPrintHeader(SEMP_PARSE_STATE* parse)
{
    ...
    if (print)
    {
        ...
        sempPrintf(print, "     %5d: Message Length (bytes)",
...
bool sempUnicoreBinaryReadCrc(SEMP_PARSE_STATE* parse, uint8_t data)
{
    ...
    else
    {
        sempPrintf(
            ...
            parse->parserName, parse->buffer[parse->length - 4],
            parse->buffer[parse->length - 3], parse->buffer[parse->length - 2],
            parse->buffer[parse->length - 1], scratchPad->unicoreBinary.crc & 0xff,
...
bool sempUnicoreBinaryBinarySync3(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    // Verify sync byte 3
    if (data != 0xB5)
...
bool sempUnicoreBinaryBinarySync2(SEMP_PARSE_STATE* parse, const uint8_t data)
{
    // Verify sync byte 2
    if (data != 0x44)
        // Invalid sync byte, start searching for a preamble byte
...
bool sempUnicoreBinaryPreamble(SEMP_PARSE_STATE* parse, const uint8_t data)
{
