...
bool sempSbfReadBytes(SEMP_PARSE_STATE *parse, uint8_t data) {
  ...
  scratchPad->sbf.computedCRC =
      semp_ccitt_crc_update(scratchPad->sbf.computedCRC, data);
  ...
  if (scratchPad->sbf.bytesRemaining == 0) {
    ...
    if ((scratchPad->sbf.computedCRC == scratchPad->sbf.expectedCRC) ||
      ...
      sempPrintf(parse->printDebug,
                 ...
                 parse->parserName, scratchPad->sbf.sbfID, parse->length,
                 parse->length);
...
bool sempSbfLengthMSB(SEMP_PARSE_STATE *parse, uint8_t data) {
  ...
  if (scratchPad->sbf.length % 4 == 0) {
    scratchPad->sbf.bytesRemaining =
        scratchPad->sbf.length - 8; // Subtract 8 header bytes
  ...
  sempPrintf(parse->printDebug,
             ...
             parse->parserName, parse->length, parse->length);
...
void sempSbfSetInvalidDataCallback(
    const SEMP_PARSE_STATE *parse,
