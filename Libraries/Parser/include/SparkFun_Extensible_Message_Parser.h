...
#define ASSERT_POINTER_VALID(pointer) if (pointer == nullptr) { log_e("Pointer is nullptr, %d, %s", __LINE__, __FUNCTION__); while(1);}
//----------------------------------------
// Constants
//----------------------------------------
...
SEMP_PARSE_STATE * sempBeginParser(const SEMP_PARSE_ROUTINE *parseTable, \
                                   ...
                                   const char *name, \
                                   SEMP_CUSTOM_ERROR_PRINT_CALLBACK printError = debugPrintCallback,
