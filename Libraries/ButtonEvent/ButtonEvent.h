...
#undef EVENT_DEF
  ...
  public:
    ButtonEvent(uint16_t longPressTime = 500, uint16_t longPressTimeRepeat = 200, uint16_t doubleClickTime = 200);
    void EventAttach(FuncCallback_t function);
    void EventMonitor(bool isPress);

    const char*
    GetEventString(uint16_t event) {
...
#undef EVENT_DEF
    ...
    operator uint8_t() { return priv.nowState; };

  private:
    typedef enum { STATE_NO_PRESS, STATE_PRESS, STATE_LONG_PRESS } State_t;

    typedef enum { CLICK_STATE_NONE, CLICK_STATE_WAIT_DOUBLE } ClickState_t;
